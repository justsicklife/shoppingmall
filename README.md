...
```
1. 회원은 "일반 회원가입" 과 "소셜 회원가입" 중 선택하여 가입 할 수 있습니다.
2. "소셜 회원가입"을 이용해 가입 하려는 회원은 소셜 정보를 이용해 회원가입 및 로그인이 가능합니다.
3. "일반 회원가입"을 이용해 이메일 인증 후 가입한 회원은 아이디와 비밀번호를 통해 로그인합니다.
4. 회원은 마이페이지에서 개인 정보를 열람 및 수정 가능 하며, 비밀번호 변경, 회원 탈퇴를 할 수 있습니다.
5. 관리자는 상품 게시를 할수 있습니다. (상품 이름, 상품 색상, 상품 사이즈, 상품 설명, 상품 사진)
