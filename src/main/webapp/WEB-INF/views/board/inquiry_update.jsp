...
<body>
	<main class="container py-5" style="min-height: 730px;">
		...
		<hr>
		<br>
		<form action="/board/inquiryUpdate.do" method="post" >
			<c:choose>
				<c:when test="${detail.boardMemberIdx == user}">
					...
					<div class="row">
						<div class="col-3 mb-3">
							<select class="form-select" id="category" name="boardCategory" required="required">
								...
								<option value="배송" ${detail.boardCategory eq '배송' ? 'selected' : ''}>배송</option>
								<option value="상품 문의" ${detail.boardCategory eq '상품 문의' ? 'selected' : ''}>상품 문의</option>
					...
					<div class="input-group mb-3">
						<input type="text" class="form-control" name="boardTitle" maxlength="33"
							id="title" value="${detail.boardTitle}" placeholder="제목을 입력하세요" required="required">
					</div>
					<div id="write" class="mb-3 justify-content-center" style="background-color:#fff;">
						<textarea name="boardContent" id="editorTxt" class="form-control" rows="13" cols="10" 
						placeholder="내용을 입력하세요" required="required">${detail.boardContent}</textarea>
					</div>
					<div class="row">
...
<script src="/resources/js/board/inquiry_update.js"></script>
